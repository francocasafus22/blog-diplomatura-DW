...
export default function CommentCard({comment, slug}) {
    ...
    return (
        <div className="flex space-x-2 bg-secondary rounded-lg p-5 text-secondary-foreground gap-2 relative" key={comment._id}>
            {comment.isOwner && <Dialog open={open} onOpenChange={setOpen}>
                ...
                <DialogContent className="sm:max-w-[425px]">                
                    ...
                    <Button variant={"destructive"} onClick={()=>{mutate({commentId: comment._id})}}>
                        {isPending ? <Loading color="border-white" className={"w-4! h-4! border-2!"}/> :"Delete"}
                    </Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>}
